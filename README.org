...
- I whitelist packages from [[http://melpa.milkbox.net][melpa]] (melpa-unstable, actually). Modify variable "melpa-include-packages" in init-elpa.el if prefer some package from it.
...
- If you don't want to use Emacs' own package manager, please extract the your package's source code at ~/.emacs.d/site-lisp. There are already samples there.
...
If you need a high level guide on how to become a Emacs master. Please check my [[https://github.com/redguardtoo/mastering-emacs-in-one-year-guide][Mastering Emacs in one year]].
...
Please note that all your packages are locked. So you *cannot* upgrade package online in the future.
